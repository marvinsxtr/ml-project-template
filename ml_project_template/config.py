...
from ml_project_template.utils import ConfigKeys, get_output_dir, git_commit_hash, logger
...
def pre_call(
    ...
    prepare_fn: Callable[[DictConfig], None] | None = None,
    verbose: bool = False,
) -> None:
    ...
    Args:
        ...
        prepare_fn: Function to use for preparing the config.
    ...
    if ConfigKeys.COMMIT in config.keys():  # noqa: SIM118
        OmegaConf.update(config, ConfigKeys.COMMIT, git_commit_hash())
...
def run(
    ...
    zen(
        main_function,
        pre_call=partial(pre_call, seed_fn=seed_fn, prepare_fn=prepare_fn, verbose=verbose),
